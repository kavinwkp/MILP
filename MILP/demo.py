...
for index in range(0, N-1):
    m.addConstr((v_point_squ_x[index + 1] - v_point_squ_x[index]) == 2 * a_i[index + 1] * dd, name="get a_i")  # 2
for index in range(1, N):
    m.addConstr(delTime[index] == dd * v_ave_div_x[index], name="get delTime")  # 6
    m.addConstr(f_drag[index] == 0.5 * rho * Sfp * v_ave_squ_x[index], name="drag formula")

m.addConstr(quicksum(delTime) <= Time_total)    # 8

for index in range(1, N):
    m.addConstr(E_m[index] - 0.5 * mass * (v_point_squ_x[index] - v_point_squ_x[index - 1]) - f_drag[index] * dd - mass * g * delH >= 0, name="E_m")   # 11
    m.addConstr(E_i[index] <= P_BAT * dd * v_ave_div_x[index])  # 14
    m.addConstr(E_i[index] * n_BAT <= F_max * dd)   # 16
